...
try:
    ...
    # 🚀 自動比對：無視空格的大小寫比對
    target_sheet = next((s for s in xl.sheet_names if s.strip() == st.session_state.current_user), None)
        ...
        for _, row in user_df.iterrows():
            ...
            try:
                ...
                calendar_events.append({
                    "title": f_no, "start": start_date.strftime('%Y-%m-%d'),
                    "end": (end_date + timedelta(days=1)).strftime('%Y-%m-%d'), "allDay": True
                })
            except: continue
    else:
        st.sidebar.warning(f"找不到分頁：{st.session_state.current_user}")
